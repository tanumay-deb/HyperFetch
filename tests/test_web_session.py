...
def test_session_reports_the_client_is_off(client):
    """What the page needs to tell the three states apart."""
    r = _get(client, "/api/session")
    assert r.get_json() == {"enabled": False, "hasPassword": False,
                            "authed": False}


def test_login_is_refused_while_it_is_switched_off(client):
    r = _post(client, "/api/login", json={"username": "admin", "password": "anything"})
...
def test_the_right_password_signs_you_in(client):
    ...
    web_auth.set_enabled(True)
    r = _post(client, "/api/login", json={"username": "admin", "password": PW})
...
def test_the_wrong_password_does_not(client):
    web_auth.set_password(PW)
    web_auth.set_enabled(True)
    r = _post(client, "/api/login", json={"username": "admin", "password": "wrong one here"})
    assert r.status_code == 401
    assert r.get_json()["code"] == "bad-login"
...
def test_logout_ends_the_session(client):
    web_auth.set_password(PW)
    web_auth.set_enabled(True)
    _post(client, "/api/login", json={"username": "admin", "password": PW})
...
def test_repeated_guesses_get_locked_out(client):
    ...
    for _ in range(web_auth.MAX_ATTEMPTS):
        _post(client, "/api/login", json={"username": "admin", "password": "nope nope nope"})
    r = _post(client, "/api/login", json={"username": "admin", "password": "nope nope nope"})
...
def test_the_lockout_is_per_device(client):
    ...
    for _ in range(web_auth.MAX_ATTEMPTS + 1):
        _post(client, "/api/login", json={"username": "admin", "password": "nope nope nope"}, addr="192.168.1.9")
    r = _post(client, "/api/login", json={"username": "admin", "password": PW}, addr="192.168.1.77")
